...
const getNumQueryFunc = loading => query => ({// 一码通查询
  type: 'organizationAccount/getNumQuery',
  payload: query || {},
  loading,
});
...
const mapStateToProps = state => ({
  ...
  numQuery: state.organizationAccount.numQuery,
});
...
export default class SearchHome extends PureComponent {
    ...
    const getStepCache = (props) => {
      const promise = new Promise((resolve, reject) => {
        try {
          ...
          const load = false;
          const nowBdid = load ? '1197251' : (query.bdid || '');
          if (nowBdid) {
            getStepCacheFunc({
              ...query,
              bdid: nowBdid,
  ...
  netClick() {
    ...
    if (!_.isEmpty(returnOpinion)) {
      ...
      push('/organizationAccount/openAgent');
      return;
    }
